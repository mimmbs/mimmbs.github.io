class dbr {
    ...
    LanguagePack() {
        if (this.lang == "tr-TR") {
            return {
                ...
                txt_non_tra: "Bu cari hesaba ait işlem kaydı yok.",
    ...
    async SaveCustomers(edited, user) {
        await this.LogTransaction('E', null, edited, user);
    ...
    async SaveTransactions(customer, transactions, user, crud) {
        await this.LogTransaction(crud, customer, transactions, user);
        await dbr.Post("savetransactions", { customer: customer, transactions: transactions });
    }

    async LogTransaction(crud, customer, editedItem, user) {
        const now = new Date().toISOString().split('T');
        const log = { crud: crud, date: `${now[0]} ${now[1].substring(0, 5)}`, customer: customer, editedItem: editedItem, user: user };
