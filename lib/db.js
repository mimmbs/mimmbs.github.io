class dbr {
    ...
    static async Reader(path) {
        const response = await fetch(`${dbr.ServerUrl}/${path}`);
        return JSON.parse(await response.text());
    }

    static async Writer(path, data) {
    ...
    async Server() {
        return await dbr.Reader("info");
    }

    async Customers() {
        return await dbr.Reader("customers");
    }

    async SaveCustomers(edited) {
        await dbr.Writer('savecustomers', edited);
